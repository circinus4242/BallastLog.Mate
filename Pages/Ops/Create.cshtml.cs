...
public class CreateModel : PageModel
{
    ...
    public class LegVm
    {
        ...
        public int Current { get; set; }  // before operation
        public int Max { get; set; }
        public int Delta { get; set; }    // user input (>=0)
    }
    ...
    private static int DistributeRemainderKeepTotal(int desiredTotal, List<LegVm> legs, int lockIndex, bool sideFrom)
    {
        ...
        Func<LegVm, int> capFn = sideFrom ? new Func<LegVm, int>(FromCap)
                                         : new Func<LegVm, int>(ToCap);
        ...
        for (int k = 0; k < n; k++)
        {
            ...
            int want = q + (k < r ? 1 : 0);
            int cap = capFn(x.l);
            int add = Math.Min(want, Math.Max(0, cap));
            x.l.Delta = add;
            assigned += add;
        }
    ...
    private static int DistributeEqual(int desiredTotal, List<LegVm> legs, bool sideFrom)
    {
        ...
        {
            int cap = capFn(legs[i]);
            int room = cap - legs[i].Delta;
